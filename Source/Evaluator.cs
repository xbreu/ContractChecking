    ...
    public IResult Evaluate(Expression eu, Context context)
    {
        ...
        switch (e)
        {
            ...
            case MultiSetFormingExpr expr:
            {
                ...
                return e0 is SequenceResult se
                    ? new MultisetResult(se.Value)
                    : new MultisetResult(((SetResult)e0).Value);
            }
            // Unary expressions
            case UnaryOpExpr expr:
            {
                ...
                return (expr.WasResolved(), rOp, expr.Op) switch
                {
                    ...
                    (true, UEO.SeqLength, _) or (true, UEO.SetCard, _) or (true, UEO.MultiSetCard, _)
                        or (true, UEO.MapCard, _)
            ...
            case BinaryExpr expr:
            {
                ...
                return (expr.WasResolved(), rOp, expr.Op) switch
                {
                    ...
                    (true, BEO.Lt or BEO.LtChar, _) or (false, _, BEOU.Lt) => ((IOrderableResult)e0).Lt(
                        (IOrderableResult)e1),
                    (true, BEO.Le or BEO.LeChar, _) or (false, _, BEOU.Le) => ((IOrderableResult)e0).Le(
                        (IOrderableResult)e1),
                    (true, BEO.Ge or BEO.GeChar, _) or (false, _, BEOU.Ge) => ((IOrderableResult)e0).Ge(
                        (IOrderableResult)e1),
                    (true, BEO.Gt or BEO.GtChar, _) or (false, _, BEOU.Gt) => ((IOrderableResult)e0).Gt(
                        (IOrderableResult)e1),
                    ...
                    // sets, multi-sets
                    (true, BEO.ProperSubset or BEO.ProperMultiSubset, _) => ((IBagResult)e0).ProperSubset(
                        (IBagResult)e1),
                    ...
                    (true, BEO.ProperSuperset or BEO.ProperMultiSuperset, _) => ((IBagResult)e0).ProperSuperset(
                        (IBagResult)e1),
                    ...
                    (true, BEO.Intersection or BEO.MultiSetIntersection, _) => ((IBagResult)e0).Intersection(
                        (IBagResult)e1),
                    (true, BEO.SetDifference or BEO.MultiSetDifference, _) => ((IBagResult)e0).Difference(
                        (IBagResult)e1),
                    // sets, multi-sets, sequences, maps
                    (true, BEO.InSet or BEO.InMultiSet or BEO.InSeq or BEO.InMap, _) =>
                        ((ICollectionResult)e1).Contains(e0),
            ...
            case SeqSelectExpr expr:
            {
                ...
                if (se.E0 != null)
                {
                    ...
                    if (se.SelectOne) return ((SequenceResult)seq).At(e0);
                }

                var e1 = (IntegerResult)seq.Length().Sub(e0);
                if (se.E1 != null) e1 = (IntegerResult)Evaluate(se.E1, context);
            ...
            case FunctionCallExpr expr:
            {
                ...
                var i = 0;
                foreach (var argName in names) dict[argName] = args[i++];
