...
using DafnyRepair.Fixes.Configuration;
...
public class FixGeneration
{
        ...
        foreach (var (methodName, enter, e) in invariants)
            if (e is BinaryExpr { Op: BinaryExpr.Opcode.Iff } be)
            {
                    ...
                    e00.Name == passingString)
                {
                    ...
                    (val ? iP : iF).Add((m, strengthening ? ContractType.PRE_CONDITION : ContractType.POST_CONDITION,
                        be.E1));
                }
                    ...
                    e01.Name == passingString)
                {
                    ...
                    (val ? iF : iP).Add((m, strengthening ? ContractType.PRE_CONDITION : ContractType.POST_CONDITION,
                        be.E1));
                }
                ...
                if (be.E0 is BinaryExpr
                    {
                        Op : BinaryExpr.Opcode.Eq, E0: IdentifierExpr { Name: "pre_condition" }
                    } eq02)
                {
                ...
                if (be.E0 is BinaryExpr
                    {
                        Op : BinaryExpr.Opcode.Neq, E0: IdentifierExpr { Name: "pre_condition" }
                    } neq02)
                {
    ...
    public async Task<(List<ContractManager>, List<ContractManager>)> Weakening(SequenceResult trace)
    {
        ...
        if (FixConfiguration.ShouldDebug(DebugInformation.ACTION_RUNTIMES)) sw = Stopwatch.StartNew();
        ...
        foreach (var newContracts in phi) Console.WriteLine(newContracts);
    ...
    public (bool, string) GenerateDaikonInput(SequenceResult trace, ContractManager contractManager)
    {
        ...
        foreach (var val in trace.Value)
        {
            ...
            if (currentInvalid)
            {
                if (currentOuterNonce == nonce) currentInvalid = false;
        ...
        ContractManager contractManager)
    {
        ...
        await DaikonConfiguration.WriteFilesAsync();
