...
public class DaikonTrace
{
        ...
        ContractManager contractManager)
    {
        ...
        if (!declared.Contains(name)) AddMethod(method);
        ...
        {
            if (invalidTest) return TestResult.INVALID;
    ...
    public static string GetModuleName(MemberDecl method)
    {
        ...
        foreach (var item in items.Take(items.Length - 2))
        {
            ...
            if (item != "_module") result.Append("_Compile");
    ...
    public static string GetClassName(MemberDecl method)
    {
        ...
        if (className.StartsWith("__")) className = string.Join("", className.Skip(2)) + "__";
