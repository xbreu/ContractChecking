...
public class MapResult : ATableResult<IResult>
{
    ...
    public override string ToDaikonInput()
    {
        ...
        result.Append("[ ");
        foreach (var p in Value) result.Append($"[ {p.Key.ToDaikonInput()} {p.Value.ToDaikonInput()} ] ");
    ...
    public MapResult Merge(MapResult other)
    {
        ...
        foreach (var y in other.Value) result[y.Key] = y.Value;
    ...
    {
        var result = Value.ToDictionary(x => x.Key, x => x.Value, ResultComparer.Get());
        foreach (var key in keys.Value) result.Remove(key);
